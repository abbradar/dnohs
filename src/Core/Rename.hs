...
renameTypes typs = typs & I.itraverseSet %%~ rn
  where rn :: Ann Pos (TyDeclD Name Name) -> Compiler (Ann Pos (TyDeclD QName Name))
        ...
        rnPar :: Map (TyVar Name) (TyVar QName) -> NQType -> Compiler QType
...
renameExpr constrs types = tr M.empty
  where rnAnn :: Pos -> Maybe NQType -> Compiler (a -> Ann' (Pos, Maybe QType) a)
        rnAnn pos ann = do
          ann' <- case ann of
            Nothing -> return Nothing
            Just a -> Just <$> renameType types a
          return $ Ann (pos, ann')
        ...
        trPat (Ann (pos, ann) pat) = (second <$> rnAnn pos ann) <*> case pat of
          PVar name -> do
            v <- newVar name
